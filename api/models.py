...
from api.models import StockAnalysis
...
class ModelTrainingRequest(BaseModel):
    ...
    symbols: Optional[List[str]] = Field(
        None, description="List of stock symbols to train models for. If None, uses popular stocks."
    )
    model_type: ModelType = Field(ModelType.RANDOM_FOREST, description="Type of ML model to train")
    period: Period = Field(Period.ONE_YEAR, description="Data period for training")
    save_model: bool = Field(False, description="Whether to save the trained model to disk")
...
class PortfolioAnalysisRequest(BaseModel):
    ...
    stocks: List[PortfolioStock] = Field(
        ..., min_items=1, description="List of stocks in the portfolio"
    )
    analysis_type: AnalysisType = Field(
        AnalysisType.PERFORMANCE, description="Type of analysis to perform"
    )


class BatchAnalysisType(str, Enum):
    """Types of batch analysis."""
...
class OutputFormat(str, Enum):
    """Output format for batch analysis."""
...
class BatchAnalysisRequest(BaseModel):
    ...
    symbols: List[str] = Field(
        ..., min_items=1, max_items=50, description="List of stock symbols to analyze"
    )
    period: Period = Field(Period.ONE_YEAR, description="Data period for analysis")
    analysis_type: BatchAnalysisType = Field(
        BatchAnalysisType.FULL, description="Type of analysis to perform"
    )
    output_format: OutputFormat = Field(OutputFormat.JSON, description="Output format for results")
...
class SystemStatus(BaseModel):
    ...
    cache: str = Field(..., description="Cache system status")
    external_apis: str = Field(..., description="External API status")
...
class HealthResponse(BaseModel):
    ...
    system_status: Union[SystemStatus, Dict[str, Any]] = Field(
        ..., description="System component status"
    )
    performance_metrics: Union[PerformanceMetrics, Dict[str, Any]] = Field(
...
# Additional helper models that might be needed


class ModelTrainingResult(BaseModel):
    """Result of model training for a single symbol."""
...
class ModelTrainingResponse(BaseModel):
    ...
    trained_models: List[ModelTrainingResult]
...
async def analyze_stock(
    ...
    if _ is None:
        _ = await check_rate_limit()
    if request_info is None:
        request_info = await log_request_middleware()
...
async def get_technical_analysis(
    ...
    if indicators is None:
        indicators = Query(None, description="Specific indicators")
    if _ is None:
        _ = await check_rate_limit()
...
async def analyze_portfolio(request: PortfolioAnalysisRequest, _: None = None):
    """
    ...
    try:
        ...
        try:
            ...
            if request.output_format == OutputFormat.CSV:
                ...
                # csv_data variable removed as it was unused
